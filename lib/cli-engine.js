    ...
    if (pluginNames) {
        pluginNames.forEach(function (pluginName) {
            var pluginNameWithoutPrefix = removePluginPrefix(pluginName),
                pluginNameWithPrefix = PLUGIN_NAME_PREFIX + pluginNameWithoutPrefix,
                pluginPath,
                plugin;
            ...
            if (!loadedPlugins[pluginNameWithoutPrefix]) {
                ...
                try {
                    // try to resolve the plugin from cwd
                    pluginPath = resolve.sync(pluginNameWithPrefix, {basedir: basedir});
                } catch (error) {
                    // maybe the plugin is installed globally
                    pluginPath = pluginNameWithPrefix;
                }
